    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_pool_project is not None:
            ...
            if n_points > 0:
                ...
                sampled = torch.empty((bs, self.d_model, 0, 1), device=points.device, dtype=img_feats.dtype)
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_pool_project is not None:
            ...
            if n_boxes > 0:
                ...
                sampled = torch.empty((0, self.d_model, self.roi_size, self.roi_size), device=boxes_xyxy.device, dtype=img_feats.dtype)
